...
if [ `so` = "linux" ]; then
    if [ -x "$(command -v pacman)" ]; then
        ...
        wget --no-check-certificate -q https://github.com/neovim/neovim/releases/download/stable/nvim-linux64.tar.gz -O ~/.local/nvim-linux64.tar.gz
        # wget --no-check-certificate -q https://github.com/neovim/neovim/releases/download/v0.8.3/nvim-linux64.tar.gz -O ~/.local/nvim-linux64.tar.gz
